...
videoWrite = cv2.VideoWriter(r'test.mp4', fourcc, 8, (1000, 1000))
########################################################################################################
...
def run_rnn(tokens, newline_adj=0):
    ...
    out, all_ffn_out, model_state = model.forward(tokens, model_state)
...
def on_message(message):
    ...
    if msg == '+reset':
    ...
    elif msg[:5].lower() == '+gen ' or msg[:4].lower() == '+qa ' or msg[
        ...
        if msg[:5].lower() == '+gen ':
            ...
            model_state = None
            model_tokens = []
            out, all_ffn_out = run_rnn(tokenizer.encode(new))
        ...
        for i in range(FREE_GEN_LEN + 100):
            ...
            if msg[:4].lower() == '+qa ':  # or msg[:4].lower() == '+qq ':
                out, all_ffn_out = run_rnn([token], newline_adj=-2)
            else:
                out, all_ffn_out = run_rnn([token])
        ...
        for i in range(999):
            ...
            out, all_ffn_out = run_rnn([token], newline_adj=newline_adj)
            ...
            if len(all_ffn_out[1].shape) == 1:
                ...
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                send_msg_y=100
                if len(send_msg)//20>1:
                    for send_msg_index in range(len(send_msg)//20):
                        if send_msg_index==0:
                            cv2.putText(img, send_msg[0:20], (10, send_msg_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                        else:
                            if send_msg_index==len(send_msg)//20:
                                cv2.putText(img, send_msg[send_msg_index * 20 + 20:],
                                            (10, send_msg_y+35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                            cv2.putText(img, send_msg[send_msg_index*20:send_msg_index*20+20], (10, send_msg_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                        send_msg_y+=35

                else:
                    cv2.putText(img, send_msg, (10, send_msg_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
